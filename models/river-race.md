...
all four listed the same five clans as `clans[]`). So an earlier section's entry is each current rival's own result on
that day, in whatever bracket it raced then, not the current bracket's history; a recorder keying rows by bracket must
scope to the current section for that reason too, not only to avoid double counting.
